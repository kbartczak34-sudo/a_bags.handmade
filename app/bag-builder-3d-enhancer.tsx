...
function assetUrls(modelId: string) {
  const encoded = encodeURIComponent(modelId.trim());
  const root = `/3d/bags/${encoded}`;
  return {
...
async function assetsExist(urls: ReturnType<typeof assetUrls>) {
  const results = await Promise.all(
    REQUIRED_ASSET_KEYS.map(async (key) => {
      try {
        const response = await fetch(urls[key], { method: "HEAD", cache: "no-store" });
        return response.ok;
      } catch {
        return false;
      }
...
function materialKind(meshName: string) {
  ...
  if (name.includes("accessor")) return "cord" as const;
...
async function mountThreeTwin(layer: HTMLElement) {
  ...
  const family = layer.dataset.family ?? "";
  const modelId = familyModelId(family);
  ...
  const [{ default: THREE }, { GLTFLoader }, { RoomEnvironment }, { createABagsThreePbrMaterial }] = await Promise.all([
    import("three"),
  ...
  key.castShadow = true;
  scene.add(key);
  const fill = new THREE.DirectionalLight(0xf3e5da, 0.85);
  fill.position.set(-4, 1.8, 3.2);
  scene.add(fill);
  const rim = new THREE.DirectionalLight(0xffffff, 1.4);
  rim.position.set(1.5, 4.0, -4.0);
  scene.add(rim);
  ...
  const loadModel = async () => {
    ...
    try {
      ...
      if (disposed || token !== loadToken) {
        disposeObject(THREE, gltf.scene);
        return;
      }
      root = gltf.scene;
      root.traverse((object) => {
        const mesh = object as import("three").Mesh;
        if (!mesh.isMesh) return;
        const name = mesh.name || "body";
        mesh.castShadow = true;
        mesh.receiveShadow = true;
        const kind = materialKind(name);
        mesh.material = createABagsThreePbrMaterial(kind, layer.dataset.color || "#E8DDCC");
      });
      scene.add(root);
  ...
  const animate = () => {
    ...
    if (root) root.rotation.y += 0.0009;
  ...
  const observer = new MutationObserver(() => {
    ...
    root.traverse((object) => {
      ...
      materials.forEach((material) => {
        ...
        if (physical.color) physical.color.set(nextColor);
      });
    });
  });
...
export default function BagBuilder3DEnhancer() {
  useEffect(() => {
    ...
    const enhance = (layer: HTMLElement) => {
      ...
      let twinCleanup: (() => void) | null = null;
      let twinStarted = false;
      const startTwin = () => {
        if (twinStarted) return;
        twinStarted = true;
        void mountThreeTwin(layer).then((cleanup) => {
          twinCleanup = cleanup;
        }).catch(() => {
          twinCleanup = null;
          layer.dataset.abagsThreeFallback = "runtime-error";
          layer.removeAttribute(THREE_LOADING_ATTR);
        });
      };
      startTwin();
      ...
      const cleanup = () => {
        viewListeners.forEach((cleanupView) => cleanupView());
    ...
    const scheduleScan = () => {
      if (scanTimer !== null) window.cancelAnimationFrame(scanTimer);
      scanTimer = window.requestAnimationFrame(() => {
        scanTimer = null;
        scan();
      });
    };
