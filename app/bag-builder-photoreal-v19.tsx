...
function disk(cx:number,cy:number,cz:number,rx0:number,ry0:number):Mesh{const p:number[]=[],n:number[]=[],u:number[]=[];const N=64;for(let i=0;i<N;i+=1){const t=i*2*Math.PI/N,j=(i+1)*2*Math.PI/N;const center:V3=[cx,cy,cz],next:V3=[cx+rx0*Math.cos(j),cy+ry0*Math.sin(j),cz],current:V3=[cx+rx0*Math.cos(t),cy+ry0*Math.sin(t),cz];tri(p,n,u,center,next,current,[0,0,1],[.5,.5],[j/N,1],[i/N,1]);}return {p,n,u};}
