...
function classifyStripeError(status: number, error: { code?: string; type?: string; message?: string } | undefined) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return json({ error: "Nieprawidłowe dane płatności.", code: "INVALID_JSON" }, 400);
  }
  ...
  if (!snapshot) {
    return json({ error: "Production Snapshot nie istnieje lub wygasł.", code: "PRODUCTION_SNAPSHOT_NOT_FOUND" }, 404);
  }

  const productionPackage = snapshot.package;
  if (!isObject(productionPackage)) {
    return json({ error: "Production Snapshot jest nieprawidłowy.", code: "PRODUCTION_SNAPSHOT_INVALID" }, 409);
  }
  const pricing = productionPackage.pricing;
  if (!isObject(pricing) || pricing.status !== "AVAILABLE" || pricing.currency !== "PLN" || !Number.isInteger(pricing.grossCents) || (pricing.grossCents as number) < 1) {
    return json({ error: "Cena konfiguracji nie jest dostępna do bezpiecznej płatności.", code: "SNAPSHOT_PRICING_UNAVAILABLE" }, 409);
  }
  ...
  try {
    ...
    if (isProductionHost && stripeMode !== "live") {
      return json({ error: "Płatności produkcyjne nie są jeszcze aktywne.", code: "stripe_live_required" }, 503);
    }
    if (isProductionHost && !isStripeLiveWebhookReady()) {
      return json({ error: "Webhook płatności produkcyjnych nie został jeszcze potwierdzony.", code: "stripe_live_webhook_required" }, 503);
    }
    ...
    try {
      ...
      console.warn("Configurator checkout pickup settings unavailable", {
    ...
    try {
      ...
      // Keep the generic error response below.
    }
    if (!response.ok) {
    ...
    if (!stripeBody.url) {
      return json({ error: "Stripe utworzył sesję bez adresu przekierowania.", code: "stripe_missing_url", requestId }, 502);
    }
    return json({ url: stripeBody.url, snapshotId: snapshot.id, productionPackageHash: snapshot.packageHash });
  } catch (error) {
    if (error instanceof StripeConfigurationError) {
      return json({ error: "Płatności Stripe nie mają skonfigurowanego klucza w środowisku produkcyjnym.", code: "stripe_configuration_error" }, 503);
    }
