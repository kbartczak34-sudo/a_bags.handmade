...
export type SiteContent = {
  announcement: { visible: boolean; primary: string; secondary: string };
  brand: { name: string; descriptor: string };
  navigation: { collection: string; story: string; reviews: string; contact: string; cart: string };
  hero: { badge: string; title: string; accent: string; lead: string; primaryCta: string; secondaryCta: string; noteOne: string; noteTwo: string; noteThree: string; imageAlt: string; imageLabel: string; imageSublabel: string; imageCaption: string };
  collection: { eyebrow: string; title: string; noteLabel: string; noteText: string; emptyTitle: string; emptyText: string };
  story: { visible: boolean; eyebrow: string; title: string; description: string; cta: string };
...
export type SiteContentPayload = { content: SiteContent; heroImageUrl: string };

export const defaultSiteContent: SiteContent = {
  announcement: {
    visible: false,
    primary: "Ręcznie tworzone w Polsce",
    secondary: "Każda torebka tworzona ręcznie",
  },
  brand: { name: "a_bags", descriptor: "handmade" },
  ...
  story: {
    ...
    description: "a_bags.handmade to mała polska marka, w której sznurek, kolor i detal spotykają się w niepowtarzalnych torebkach. Każdy egzemplarz powstaje ręcznie — dlatego nie ma dwóch idealnie takich samych.",
  ...
  benefits: {
    ...
    items: [
      { title: "100% handmade", description: "Każde oczko powstaje ręcznie w pracowni a_bags.handmade." },
      { title: "Limitowane serie", description: "Małe kolekcje sprawiają, że każda torebka jest wyjątkowa." },
      { title: "Pakowane z troską", description: "Starannie przygotowane — także jako prezent dla bliskiej osoby." },
    ],
  },
  reviews: {
    visible: true,
    ...
    emptyText: "Pierwsze prawdziwe opinie klientek pojawią się tutaj już wkrótce.",
    emptyBrand: "a_bags.handmade",
    emptyLabel: "miejsce na Wasze słowa",
    formEyebrow: "Twoje doświadczenie",
    formTitle: "Zostaw swoją opinię",
    formDescription: "Napisz kilka słów o swojej torebce. Po akceptacji opinia pojawi się w tej sekcji.",
  },
  instagram: {
    visible: true,
  ...
  theme: { ink: "#5a4245", paper: "#fbf6f2", cream: "#fffaf8", accent: "#d6a3a7", accentDark: "#b87880", accentLight: "#f4dddf" },
};
