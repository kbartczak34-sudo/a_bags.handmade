...
test("premium chain realism is mounted after calibrated accessory material finishing", () => {
  ...
  assert.match(chain, /CHAIN_VERSION = "chain-metal-v1-calibrated"/);
});
...
test("chain uses smaller denser alternating links with physical metal response", () => {
  assert.match(fidelity, /chainLinks:\s*30/);
  assert.match(chain, /const rx = 4\.05 \* scale \* center\.scale/);
  assert.match(chain, /const ry = 2\.28 \* scale \* center\.scale/);
...
test("leather shoulder section replaces links only in the calibrated centre span", () => {
  ...
  assert.match(chain, /arcIndex >= SHOULDER_START && arcIndex <= SHOULDER_END/);
