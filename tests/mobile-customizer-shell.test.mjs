...
test("mobile shell hardening is the final customizer stylesheet", () => {
  const photoTrue = customizer.indexOf('import "./bag-builder-photo-true.css"');
  const optionTruth = customizer.indexOf('import "./bag-builder-photo-true-option-truth.css"');
  const shellFix = customizer.indexOf('import "./bag-builder-mobile-shell-fix.css"');
  assert.ok(photoTrue >= 0);
  assert.ok(optionTruth > photoTrue);
  assert.ok(shellFix > optionTruth, "mobile shell fix must load after every Photo-True visual layer");
});
...
test("customizer content remains constrained inside the full-screen shell", () => {
