...
test("public catalog is built only from the strict validated body evidence chain", () => {
...
test("public evidence exposes only identifiers and customer-safe physical summary", () => {
...
test("public evidence route is GET-only, validates filters and returns no-store data", () => {
...
test("public route reads server calibration and fails closed when evidence is unavailable", () => {
  assert.match(route, /getCraftCalibrationSnapshot/);
  assert.match(route, /buildPublicCraftBodyEvidenceCatalog/);
